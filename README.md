...
```bash
python src/generate_labels.py --base-url "https://example.com/item/" --count 24
```
...
```bash
python src/generate_labels.py --base-url "https://example.com/keg/scan/" --prefix "KEG-" --count 24
```
...
```bash
python src/generate_labels.py --base-url "https://example.com/keg/scan/" --prefix "KEG-" --start 100 --count 48
```

### Custom Output Filename

```bash
python src/generate_labels.py --base-url "https://example.com/item/" --count 24 --output output/my_labels.pdf
```

### All Options

```bash
python src/generate_labels.py --base-url "https://example.com/item/" --prefix "ITEM-" --start 1 --count 24 --output output/labels.pdf --template templates/SL655.pdf
```
...
```bash
python src/generate_labels.py --base-url "https://mycompany.com/inventory/" --prefix "INV-" --count 24
```

**Generate keg tracking labels KEG-0050 through KEG-0099:**
```bash
python src/generate_labels.py --base-url "https://brewery.com/keg/scan/" --prefix "KEG-" --start 50 --count 50
```

**Generate 100 asset labels (will create 5 sheets):**
```bash
python src/generate_labels.py --base-url "https://company.com/asset/" --prefix "ASSET-" --start 1 --count 100
```
...
- Overlay them onto the SL655.pdf template
...
- The script uses exact SL655-XW specifications for accurate printing
